...
export const Header: React.FC = () => {
  ...
  // Theme bootstrapping from localStorage / OS preference
  useEffect(() => {
  ...
  // Keep <html class="dark"> in sync with toggle
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const root = document.documentElement;
    if (isDark) {
  ...
  // Tighten nav background a bit when scrolled
  useEffect(() => {
    if (typeof window === 'undefined') return;
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40">
      {/* Top promo banner - transparent over beams */}
      ...
      <div className="mt-2 pb-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div
            className={
              'flex items-center justify-between gap-3 rounded-full border border-white/12 px-3 sm:px-5 md:px-6 py-2 md:py-2.5 shadow-[0_18px_60px_rgba(15,23,42,0.9)] backdrop-blur-xl transition-colors duration-300 ' +
              (isScrolled ? 'bg-slate-950/80' : 'bg-slate-950/60')
            }
          >
            {/* Left: logo block (uses your existing Logo component) */}
      ...
      {/* Mobile nav dropdown (glass panel) */}
      {mobileOpen && (
        <div className="md:hidden px-4 sm:px-6 lg:px-8 pb-4">
          <div className="max-w-7xl mx-auto">
            <div className="rounded-3xl bg-slate-950/85 border border-white/15 backdrop-blur-xl py-4 px-4 shadow-[0_18px_60px_rgba(15,23,42,0.9)]">
